...
// ── Recommendation Card (Psychology-Informed) ─────────────────────
function RecommendationCard({ rec, rank, recipientName, onSelected, onReaction, sessionId, onBuyLinkClicked }: {
  ...
  // Slot-specific differentiators (research: explicit differences reduce comparison effort)
  const SLOT_CONFIG = [
  ...
  // Match percentage (research: Netflix-style % match reduces decision fatigue)
  ...
  return (
    <div className={`bg-white rounded-2xl border overflow-hidden animate-in transition-all ${
      isTopPick ? "border-green-300 shadow-md ring-1 ring-green-100" : "border-gray-200 shadow-sm"
    }`}>
      {/* Clickable product area — opens product page in new tab */}
      <a
        href={rec.buyUrl}
        target="_blank"
        ...
        className="block"
        onClick={() => {
          if (sessionId) trackBuyLinkClicked(sessionId, rec.id, rec.price, rec.buyUrl);
          onBuyLinkClicked?.(rec);
        }}
      >
        {/* Hero image */}
        <div className={`relative w-full bg-gray-50 ${rec.imageUrl && rec.imageUrl.startsWith("http") ? "aspect-square" : "h-36"}`}>
          ...
          {/* Match % — Netflix-style confidence */}
        ...
        <div className="px-5 pt-3 pb-2">
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="text-right flex-shrink-0">
              ...
              <p className="text-[10px] text-gray-400">{buyDomain}</p>
            </div>
          </div>
          {rec.description && (
      ...
      {/* Personalized context — merged "why" + "what it says" into one block (research: compress hierarchy) */}
      <div className="px-5 pt-1 pb-2">
        ...
        {rank === 2 && (
          <p className="text-[11px] text-purple-500 mt-2 px-1">Recipients rate unexpected gifts 40% higher than givers expect.</p>
        )}
      </div>

      {/* How to give it — collapsed */}
      <div className="px-5 pb-2" onClick={(e) => e.stopPropagation()}>
        <details onClick={(e) => e.preventDefault()}>
          <summary className="text-[11px] text-gray-400 cursor-pointer hover:text-gray-600 py-1">How to give it</summary>
          <div className="bg-amber-50 border border-amber-100 rounded-xl px-4 py-2.5 mt-1 mb-1">
      ...
      <div className="px-5 pb-4 pt-1 border-t border-gray-100">
        <div className="flex items-center gap-1 mb-3" onClick={(e) => e.stopPropagation()}>
          {!reaction ? (
            <>
              <span className="text-[10px] text-gray-300 mr-0.5">This pick:</span>
              <button onClick={() => handleReaction("relevant")} className="p-2 rounded-lg hover:bg-green-50 text-gray-300 hover:text-green-500 transition" title="Great match">
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M14 10h4.764a2 2 0 011.789 2.894l-3.5 7A2 2 0 0115.263 21h-4.017c-.163 0-.326-.02-.485-.06L7 20m7-10V5a2 2 0 00-2-2h-.095c-.5 0-.905.405-.905.905 0 .714-.211 1.412-.608 2.006L7 11v9m7-10h-2M7 20H5a2 2 0 01-2-2v-6a2 2 0 012-2h2.5" /></svg>
              </button>
              <button onClick={() => handleReaction("irrelevant")} className="p-2 rounded-lg hover:bg-red-50 text-gray-300 hover:text-red-400 transition" title="Not a match">
              ...
              <button onClick={() => handleReaction("too_expensive")} className="p-2 rounded-lg hover:bg-amber-50 text-gray-300 hover:text-amber-500 transition text-[11px] font-semibold" title="Too pricey">$$$</button>
            </>
          ) : (
            <span className="text-[11px] text-gray-400 pl-1">
              {reaction === "relevant" ? "Great match" : reaction === "irrelevant" ? "We'll adjust" : "Too pricey"}
            </span>
          )}
        </div>
