...
async function runPersona(persona: Persona): Promise<PersonaResult> {
  ...
  if (recs.length > 0 && postRecs) {
    ...
    if (postRecs.reactions) {
      ...
      for (const [slot, reaction] of Object.entries(postRecs.reactions)) {
        ...
        if (recs[slotIdx]) {
          ...
          // Track reaction as event (shows in admin Decision Journey)
          await fetch(`${BASE_URL}/api/events`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              eventType: "recs.card_reaction",
              sessionId,
              eventData: { productId: recs[slotIdx].id, productName: recs[slotIdx].name, reaction, slot: slotIdx },
            }),
          }).catch(() => {});
          // Also send to feedback API
    ...
    if (selectedRec) {
      ...
      // Track selection as event
      await fetch(`${BASE_URL}/api/events`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          eventType: "gift.marked_as_purchased",
          sessionId,
          eventData: { productId: selectedRec.id, productName: selectedRec.name, price: selectedRec.price },
        }),
      }).catch(() => {});
